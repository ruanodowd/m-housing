...
class MapVisualization:
    ...
    def add_work_marker(self, lat, lon):
        folium.Marker([lat, lon], popup="Work", icon=folium.Icon(color="green", icon="screenshot")).add_to(
            self.map
        )

    def to_iframe(self, height, width):
        self.map.get_root().width = width
        self.map.get_root().height = height
